...
import {
  txt, sep, head, kv, rankRow, bubble, flexMsg, RED, INK, MUTED, GREEN,
} from './flex.js';
...
export function notice(title, sub, lines, opts = {}) {
  ...
  const lead = lines.filter(Boolean).join('\n');
  return [lead, flexMsg(title, bubble(title, sub, body))];
}
...
export function orderAck(name, added, orders, leftCount) {
  ...
  return [lead, flexMsg(title, bubble(title, `${orders.length} รายการที่สั่งแล้ววันนี้`, body))];
}
...
export function answer(kind, snap, orders) {
  ...
  switch (kind) {
    ...
    case 'cash': {
      if (!c.hasData) {
        return notice('ยังบอกเงินสดไม่ได้', sub, [
          `${snap.dayLabel} ยังไม่มีไฟล์เงินโอน`,
          `ยอดขายทั้งวัน ${baht(s.total)} บาท`,
          'อัปไฟล์ธนาคารเข้าระบบแล้วจะคำนวณเงินสดให้ทันที',
        ], { hot: true });
      }
    ...
    case 'transfer': {
      if (!c.hasData) {
        return notice('ยังไม่มีข้อมูลเงินโอน', sub, [
          `${snap.dayLabel} ยังไม่ได้อัปไฟล์ธนาคารของวันนี้`,
          'วางไฟล์ที่หน้าอัปโหลดแล้วตัวเลขจะขึ้นเอง',
        ], { hot: true });
      }
    ...
    case 'help': {
      ...
      return [lead, flexMsg('คุยอะไรกับบอทได้บ้าง',
                            bubble('คุยอะไรได้บ้าง', 'พิมพ์เป็นภาษาคนได้เลย', b))];
    }
