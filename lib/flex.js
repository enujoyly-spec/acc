...
/**
 * ประโยคตอบที่อยู่ "ในกล่อง" บนสุด — เดิมส่งเป็นข้อความแยกก่อนกล่อง
 * แต่ผู้ใช้ย้ำว่าทุกคำตอบต้องเป็นกล่อง จึงย้ายเข้ามาไว้หัวเนื้อกล่องแทน
 */
export const lead = (text) =>
  txt(text, { size: 'sm', color: INK, wrap: true });

/** บรรทัดเตือนท้ายกล่องว่าข้อมูลไม่ใช่ของวันนี้ */
export const staleNote = (text) => ({
  type: 'box', layout: 'vertical', margin: 'lg',
  contents: [
    { type: 'separator', color: LINE_C },
    txt(text, { size: 'xxs', color: MUTED, margin: 'md', wrap: true }),
  ],
});
