...
const PROMPT = [
  'รูปนี้เป็น "รายงานสรุปนำส่งเงินปิดกะ" ของร้านค้าไทย (มีทั้งตัวพิมพ์และลายมือเขียน)',
  'อ่านรูปอย่างละเอียด แล้วตอบเป็น JSON สั้นๆ เท่านั้น ห้ามมีข้อความอื่น ห้ามใช้ code fence:',
  '{',
  ...
  '}',
  'ตัวเลขห้ามมีคอมมา ตอบแค่ JSON ก้อนเดียว สั้นที่สุด',
].join('\n');
...
export function parseResult(content) {
  ...
  return {
    ...
    transferSales: toNumber(parsed.transfer_sales),
    epayment: toNumber(parsed.epayment),
    ...
    success: parsed.success !== false &&
      (parsed.cash_sales != null || parsed.deposit != null || parsed.total != null),
